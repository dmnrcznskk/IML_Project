...
class ModelEvaluator:
    ...
    def __init__(self, class_names: Optional[Dict[int, str]] = None):
        ...
        if self.class_names:
            ...
            self.target_names = [
                self.class_names.get(i, f"Class {i}") for i in range(max_idx + 1)
            ]
    ...
    def evaluate(
        self, model, X_test: np.ndarray, y_test: np.ndarray, show_plot: bool = True
    ) -> Dict[str, Any]:
        """
        ...
        )  # Tu też warto dodać labels, żeby macierz miała stały wymiar 43x43

        if show_plot:
            self.plot_confusion_matrix(cm)

        return {"accuracy": acc, "confusion_matrix": cm, "report": report_dict}
